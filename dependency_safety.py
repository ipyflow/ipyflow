...
def tracefunc(frame, event, arg):
    ...
    if event == "call":
        ...
        if path not in tracefunc.closedset:
            ...
            for name in path:
                scope = scope.children_scope_dict[name]
            scope.frame_dict = frame.f_locals
            tracefunc.closedset.add(path)
...
class CheckDependency(ast.NodeVisitor):
    def get_right_side_dependency(self, value, extension = []):
        ...
        while checklist:
            ...
            if isinstance(node, ast.BinOp):
                ...
                if node.func.id not in test.function_call_dependency:
                    checklist.extend(node.args)
                    continue
    ...
    def visit_For(self, node):
        test.new_dependency[self.current_scope][node.target.id] = self.get_right_side_dependency(node.iter)
        for line in node.body:
            CheckDependency().visit(line)
...
class Scope:
    ...
    def create_node(self, name, current_CN):
        uid = id(self.frame_dict[name])
        node = VariableNode(name, current_CN, self, uid)
...
def test(line, cell):
    ...
    for name in test.new_dependency[scope]:
        scope.get_or_create_node(name, test.counter)

    for name in test.new_dependency[scope]:
        node = scope.get_node_by_name_current_scope(name)
