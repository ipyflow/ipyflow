...
class Namespace(Scope):
    ...
    def __init__(self, obj: Any, *args, **kwargs) -> None:
        ...
        if (
            obj is not None and not isinstance(obj, int) and id(obj) in nbs().namespaces
        ):  # pragma: no cover
    ...
    def is_garbage(self) -> bool:
        return (
            self._tombstone
            or self.obj_id not in nbs().aliases
            or self.obj_id not in nbs().namespaces
        )
    ...
    def transfer_symbols_to(self, new_ns: "Namespace") -> None:
        for dsym in list(
            ...
            try:
                inner_obj = nbs().retrieve_namespace_attr_or_sub(
